...
class LMS {
  const LMS({
    ...
    PercentileCutOff Function(PercentileCutOff percentile)? percentileOverride,
  ...
  ZScoreCutOff get standardDeviationCutOff {
    final map = ZScoreValue.values.asMap().map(
      (_, v) => MapEntry(v, standardDeviation(v.value)),
    );
    return _zScoreOverride != null ? _zScoreOverride(map) : map;
  }

  PercentileCutOff get percentileCutOff {
    final map = PercentileValue.values.asMap().map(
      (_, v) => MapEntry(v, standardDeviation(qnorm(v.value / 100))),
    );
