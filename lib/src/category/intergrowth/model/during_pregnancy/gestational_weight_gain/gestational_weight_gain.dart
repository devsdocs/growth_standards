...
class IntergrowthGestationalWeightGainForAgeData extends AgeBasedData {
  ...
  static final _singleton = IntergrowthGestationalWeightGainForAgeData._(
    _parse(),
  );

  static Map<Sex, Map<int, _IntergrowthGestationalWeightGainForAgeLMS>>
  _parse() {
    final map = _grow_gwg_nw.map((k1, v1) {
      final lms = LMS(
        l: v1['l']! as num,
        m: v1['m']! as num,
        s: v1['s']! as num,
      );
      return MapEntry(k1, _IntergrowthGestationalWeightGainForAgeLMS(lms: lms));
    });
...
class _IntergrowthGestationalWeightGainForAgeLMS extends LMSContext {
  _IntergrowthGestationalWeightGainForAgeLMS({required this.lms});
  @override
  final LMS lms;
