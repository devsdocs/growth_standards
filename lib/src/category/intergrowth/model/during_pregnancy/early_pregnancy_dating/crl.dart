...
class IntergrowthEarlyPregnancyDatingForCRLData extends LengthBasedData {
  ...
  _parse() {
    final map = _grow_early_preg_charts_crl_dating.map((k1, v1) {
      final lms = LMS(
        l: v1['l']! as num,
        m: v1['m']! as num,
        s: v1['s']! as num,
      );
      return MapEntry(k1, _IntergrowthEarlyPregnancyDatingForCRLLMS(lms: lms));
    });
...
sealed class IntergrowthEarlyPregnancyDatingForCRL extends LengthBasedResult
  ...
  @override
  num get measurementResultInDefaultUnit => age.ageInTotalDaysByNow.toDouble();
}
