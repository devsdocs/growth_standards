...
/// calculation for erf^-1(z) in the interval [0, 0.5].
final List<double> _ervInvImpAd = [
  1,
...
final List<double> _ervInvImpBn = [
...
/// Polynomial coefficients for a denominator of erfInvImp
/// calculation for erf^-1(z) in the interval [0.5, 0.75].
final List<double> _ervInvImpBd = [
  1,
...
final List<double> _ervInvImpCn = [
...
/// Polynomial coefficients for a denominator of erfInvImp
/// calculation for erf^-1(z) in the interval [0.75, 1] with x less than 3.
final List<double> _ervInvImpCd = [
  1,
...
final List<double> _ervInvImpDn = [
...
/// Polynomial coefficients for a denominator of erfInvImp
/// calculation for erf^-1(z) in the interval [0.75, 1] with x between 3 and 6.
final List<double> _ervInvImpDd = [
  1,
...
final List<double> _ervInvImpEn = [
...
/// Polynomial coefficients for a denominator of erfInvImp
/// calculation for erf^-1(z) in the interval [0.75, 1] with x between 6 and 18.
final List<double> _ervInvImpEd = [
  1,
...
final List<double> _ervInvImpFn = [
...
/// Polynomial coefficients for a denominator of erfInvImp
/// calculation for erf^-1(z) in the interval [0.75, 1] with x between 18 and 44.
final List<double> _ervInvImpFd = [
  1,
...
final List<double> _ervInvImpGn = [
...
/// Polynomial coefficients for a denominator of erfInvImp
/// calculation for erf^-1(z) in the interval [0.75, 1] with x greater than 44.
final List<double> _ervInvImpGd = [
  1,
...
double _erfImp(double z, bool invert) {
  if (z < 0) {
    if (!invert) {
      return -_erfImp(-z, false);
    }

    if (z < -0.5) {
      return 2 - _erfImp(-z, true);
    }

    return 1 + _erfImp(-z, false);
  }
  ...
  if (z < 0.5) {
    ...
    if (z < 1e-10) {
      ...
      result = (z * 1.125) +
          (z *
              Polynomial.evaluate(z, _erfImpAn) /
              Polynomial.evaluate(z, _erfImpAd));
    }
  } else if (z < 110) {
    ...
    if (z < 0.75) {
      ...
      r = Polynomial.evaluate(z - 0.5, _erfImpBn) /
          Polynomial.evaluate(z - 0.5, _erfImpBd);
      b = 0.3440242112;
    } else if (z < 1.25) {
      // Worst case absolute error found: 4.01854729e-21
      r = Polynomial.evaluate(z - 0.75, _erfImpCn) /
          Polynomial.evaluate(z - 0.75, _erfImpCd);
      b = 0.419990927;
    } else if (z < 2.25) {
      // Worst case absolute error found: 2.866005373e-21
      r = Polynomial.evaluate(z - 1.25, _erfImpDn) /
          Polynomial.evaluate(z - 1.25, _erfImpDd);
      b = 0.4898625016;
    } else if (z < 3.5) {
      // Worst case absolute error found: 1.045355789e-21
      r = Polynomial.evaluate(z - 2.25, _erfImpEn) /
          Polynomial.evaluate(z - 2.25, _erfImpEd);
      b = 0.5317370892;
    } else if (z < 5.25) {
      // Worst case absolute error found: 8.300028706e-22
      r = Polynomial.evaluate(z - 3.5, _erfImpFn) /
          Polynomial.evaluate(z - 3.5, _erfImpFd);
      b = 0.5489973426;
    } else if (z < 8) {
      // Worst case absolute error found: 1.700157534e-21
      r = Polynomial.evaluate(z - 5.25, _erfImpGn) /
          Polynomial.evaluate(z - 5.25, _erfImpGd);
      b = 0.5571740866;
    } else if (z < 11.5) {
      // Worst case absolute error found: 3.002278011e-22
      r = Polynomial.evaluate(z - 8, _erfImpHn) /
          Polynomial.evaluate(z - 8, _erfImpHd);
      b = 0.5609807968;
    } else if (z < 17) {
      // Worst case absolute error found: 6.741114695e-21
      r = Polynomial.evaluate(z - 11.5, _erfImpIn) /
          Polynomial.evaluate(z - 11.5, _erfImpId);
      b = 0.5626493692;
    } else if (z < 24) {
      // Worst case absolute error found: 7.802346984e-22
      r = Polynomial.evaluate(z - 17, _erfImpJn) /
          Polynomial.evaluate(z - 17, _erfImpJd);
      b = 0.5634598136;
    } else if (z < 38) {
      // Worst case absolute error found: 2.414228989e-22
      r = Polynomial.evaluate(z - 24, _erfImpKn) /
          Polynomial.evaluate(z - 24, _erfImpKd);
      b = 0.5638477802;
    } else if (z < 60) {
      // Worst case absolute error found: 5.896543869e-24
      r = Polynomial.evaluate(z - 38, _erfImpLn) /
          Polynomial.evaluate(z - 38, _erfImpLd);
      b = 0.5640528202;
    } else if (z < 85) {
      // Worst case absolute error found: 3.080612264e-21
      r = Polynomial.evaluate(z - 60, _erfImpMn) /
          Polynomial.evaluate(z - 60, _erfImpMd);
      b = 0.5641309023;
    } else {
      // Worst case absolute error found: 8.094633491e-22
      r = Polynomial.evaluate(z - 85, _erfImpNn) /
          Polynomial.evaluate(z - 85, _erfImpNd);
      b = 0.5641584396;
    }

    final double g = stdmath.exp(-z * z) / z;
    result = (g * b) + (g * r);
  } else {
...
double _erfInvImpl(double p, double q, double s) {
  double result;
  ...
  if (p <= 0.5) {
    ...
    final double r = Polynomial.evaluate(p, _ervInvImpAn) /
        Polynomial.evaluate(p, _ervInvImpAd);
    result = (g * y) + (g * r);
  } else if (q >= 0.25) {
    ...
    final double g = stdmath.sqrt(-2 * stdmath.log(q));
    final double xs = q - 0.25;
    final double r = Polynomial.evaluate(xs, _ervInvImpBn) /
        Polynomial.evaluate(xs, _ervInvImpBd);
    result = g / (y + r);
  } else {
    ...
    final double x = stdmath.sqrt(-stdmath.log(q));
    if (x < 3) {
      // Max error found: 1.089051e-20
      const double y = 0.807220458984375;
      final double xs = x - 1.125;
      final double r = Polynomial.evaluate(xs, _ervInvImpCn) /
          Polynomial.evaluate(xs, _ervInvImpCd);
      result = (y * x) + (r * x);
    } else if (x < 6) {
      // Max error found: 8.389174e-21
      const double y = 0.93995571136474609375;
      final double xs = x - 3;
      final double r = Polynomial.evaluate(xs, _ervInvImpDn) /
          Polynomial.evaluate(xs, _ervInvImpDd);
      result = (y * x) + (r * x);
    } else if (x < 18) {
      // Max error found: 1.481312e-19
      const double y = 0.98362827301025390625;
      final double xs = x - 6;
      final double r = Polynomial.evaluate(xs, _ervInvImpEn) /
          Polynomial.evaluate(xs, _ervInvImpEd);
      result = (y * x) + (r * x);
    } else if (x < 44) {
      // Max error found: 5.697761e-20
      const double y = 0.99714565277099609375;
      final double xs = x - 18;
      final double r = Polynomial.evaluate(xs, _ervInvImpFn) /
          Polynomial.evaluate(xs, _ervInvImpFd);
      result = (y * x) + (r * x);
    } else {
      // Max error found: 1.279746e-20
      const double y = 0.99941349029541015625;
      final double xs = x - 44;
      final double r = Polynomial.evaluate(xs, _ervInvImpGn) /
          Polynomial.evaluate(xs, _ervInvImpGd);
      result = (y * x) + (r * x);
    }
  }
