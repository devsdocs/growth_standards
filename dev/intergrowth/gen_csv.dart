...
void main() {
  processIntergrowthData('intergrowth/data.json', 'intergrowth/downloads');
}

void processIntergrowthData(String dataJsonPath, String outputBasePath) {
  final models =
      Intergrowth.fromJsonList(File(dataJsonPath).readAsStringSync());
    ...
    for (final item in model.items ?? <Item>[]) {
      for (final resource in item.resources ?? <Resource>[]) {
        for (final file in resource.files ?? <FileElement>[]) {
          ...
          final resourceKey = keyValues.reverse[resource.key];
          final dirPath =
              '$outputBasePath/${model.key}/${item.key}/$resourceKey';
          final htmlFilePath = '$dirPath/$fileName.htm';
          final csvOutputPath = '$dirPath/$fileName.csv';

          processHtmlToCsv(htmlFilePath, csvOutputPath);
        }
      }
    }
  }
...
void processHtmlToCsv(String htmlFilePath, String csvOutputPath) {
  ...
  final isValidLengthCsv = cleanedData.every((ls) => ls.length == list.length);
  if (!isValidLengthCsv) {
    print(
